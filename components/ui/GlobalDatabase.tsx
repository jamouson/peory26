...
export const GlobalDatabase: FunctionComponent = () => {
  ...
  return (
    <div className="px-3">
      ...
      >
        <div className="absolute top-[17rem] size-[40rem] rounded-full bg-brand-800 blur-3xl md:top-[20rem]" />
        <div className="z-10 inline-block rounded-lg border border-brand-400/20 bg-brand-800/20 px-3 py-1.5 font-semibold uppercase leading-4 tracking-tight sm:text-sm">
        ...
        <h2
          ...
          className="z-10 mt-6 inline-block bg-gradient-to-b from-white to-brand-100 bg-clip-text px-2 text-center text-5xl font-bold tracking-tighter text-transparent md:text-8xl"
        >
            ...
            <div className="grid grid-cols-1 gap-x-10 gap-y-6 rounded-lg border border-white/[3%] bg-white/[1%] px-6 py-6 shadow-xl backdrop-blur md:grid-cols-3 md:p-8">
              {features.map((item) => (
                <div key={item.name} className="flex flex-col gap-2">
                  <h3 className="whitespace-nowrap bg-gradient-to-b from-brand-300 to-brand-500 bg-clip-text text-lg font-semibold text-transparent md:text-xl">
                    {item.name}
                  </h3>
                  <p className="text-sm leading-6 text-brand-200/40">
                    {item.description}
                  </p>
                </div>
              ))}
